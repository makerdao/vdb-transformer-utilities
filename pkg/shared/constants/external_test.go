...
var _ = Describe("External constants", func() {
	...
	Describe("CompareABIs", func() {
		...
		It("has an outer method error if the ABIs have the same constructor but differing outer methods", func() {
			medianBalABI, parseErr := eth.ParseAbi(constants.GetContractABI("MEDIAN_BAL"))
			Expect(parseErr).NotTo(HaveOccurred())
			medianBalWithoutDenyABI, parseErr := eth.ParseAbi(constants.GetContractABI("MEDIAN_BAL_WITHOUT_DENY"))
			Expect(parseErr).NotTo(HaveOccurred())

			expectedError := constants.NewMismatchedOuterMethodsError(medianBalABI, medianBalWithoutDenyABI, "deny")
		...
		It("has an outer event error if the ABIs have the same constructor and methods but differing events", func() {
			medianBalABI, parseErr := eth.ParseAbi(constants.GetContractABI("MEDIAN_BAL"))
			Expect(parseErr).NotTo(HaveOccurred())
			medianBalWithoutLogMedianPriceABI, parseErr := eth.ParseAbi(constants.GetContractABI("MEDIAN_BAL_WITHOUT_LOG_MEDIAN_PRICE"))
			Expect(parseErr).NotTo(HaveOccurred())

			expectedError := constants.NewMismatchedOuterEventsError(medianBalABI, medianBalWithoutLogMedianPriceABI, "LogMedianPrice")
